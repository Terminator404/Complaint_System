...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        ...
        var rules = {
                ...
                de_name: {rule:'required',error:'验证失败!'}};
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('department').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('department').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('department').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('department').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('department').destroy({where: where});
        $this.success();
    };
