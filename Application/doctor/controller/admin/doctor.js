...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('doctor').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('doctor').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('doctor').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('doctor').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('doctor').destroy({where: where});
        $this.success();
    };
